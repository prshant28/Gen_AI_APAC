...
- **Live Voice/Video Brain (added 2026-04-29):** Real-time bidirectional conversation with the user's Second Brain via Google Gemini Live API.
  - Backend relay at `app/live_agent.py` proxies the browser's WebSocket (`/ws/live?uid=...`) to `wss://generativelanguage.googleapis.com/.../BidiGenerateContent`. The API key (`GEMINI_LIVE_API_KEY`) lives only on the server.
  - Default model: `gemini-2.5-flash-native-audio-latest` (override via `GEMINI_LIVE_MODEL`). Setup uses camelCase v1beta JSON dialect with `outputAudioTranscription` + `inputAudioTranscription` enabled. The relay decodes upstream JSON whether it arrives in text or binary WS frames.
  - Tool schema exposes 11 tools (save_memory, capture_url, create_task, list_tasks, complete_task, create_calendar_event, create_revisit, recall_memories, create_note, create_bookmark, daily_briefing). Tool calls are dispatched server-side against the existing agents with the user's `uid` re-stamped on the asyncio context (since ContextVars don't auto-propagate across tasks).
  - Frontend: `src/lib/liveClient.ts` captures mic at 16 kHz mono PCM, plays back model audio at 24 kHz via Web Audio API, and can stream camera or screen JPEG frames. `src/components/LiveChatPanel.tsx` adds a floating "Live" button (gated by `/api/live/status` so dev environments without a key don't show a dead button) that opens a panel with mic/camera/screen toggles, transcripts, tool-call feed, and a text-input fallback.
...
- **SPA deep-link hardening v2 (2026-04-29):** Bulletproof, denylist-based. The `spa_navigation_guard` HTTP middleware in `main.py` intercepts every browser document navigation (`Sec-Fetch-Dest: document` or `Accept: text/html` without `application/json`) and serves `dist/index.html` UNLESS the path looks like an API or static asset. A path is treated as backend when it (a) is one of the explicit operator/diagnostic paths in `_ALWAYS_BACKEND_EXACT` (`/health`, `/api/health`, `/calendar.ics`, `/openapi.json`, `/docs`, `/redoc`, `/metrics`, `/robots.txt`, `/sitemap.xml`), (b) starts with an API/static prefix (`/api/`, `/assets/`, `/share/`, `/__`, `/static/`, `/_next/`), or (c) ends with a known static file extension. Everything else — current SPA routes, future SPA routes, typos, share links, judge deep links — gets the SPA shell with `X-SPA-Shell: 1` header. XHR/fetch from inside the SPA still gets normal JSON because they send `Accept: application/json`. The catch-all `/{full_path:path}` is also always registered with a friendly inline-styled HTML fallback so even a missing `dist/` cannot leak raw 404 JSON.
- **Firestore persistence strategy (2026-04-29):** `app/db.py::_get_firestore_client` is persistence-first for production. Order: (1) explicit `GOOGLE_APPLICATION_CREDENTIALS` JSON file if it exists; (2) Application Default Credentials — used automatically on Cloud Run / GCE via the metadata server, no secret upload needed (the runtime service account just needs `roles/datastore.user` on `GCP_PROJECT_ID`); (3) in-memory mock as last resort. To avoid a multi-second startup hang on Replit dev (where the GCE metadata server doesn't exist), a fast 0.4s DNS preflight on `metadata.google.internal` short-circuits to the mock immediately. The current persistence mode is exposed on `/api/health` as `persistence: "firestore" | "in-memory-mock"` for instant deploy verification, and `is_using_mock_db()` is exported from `app.db`.
